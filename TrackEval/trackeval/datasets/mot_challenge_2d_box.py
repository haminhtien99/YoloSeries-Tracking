...
class MotChallenge2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            'GT_FOLDER': os.path.join(code_path, 'data/gt/UAVDT'),  # Location of GT data
            'TRACKERS_FOLDER': os.path.join(code_path, 'data/trackers/UAVDT'),  # Trackers location
            'OUTPUT_FOLDER': None,  # Where to save eval results (if None, same as TRACKERS_FOLDER)
            'TRACKERS_TO_EVAL': None,  # Filenames of trackers to eval (if None, all in folder)
            'CLASSES_TO_EVAL': ['vehicles'],  # Valid: ['pedestrian']
            'BENCHMARK': 'UAVDT',  # Valid: 'MOT17', 'MOT16', 'MOT20', 'MOT15'
    ...
    def __init__(self, config=None):
        ...
        self.class_name_to_class_id = {'car': 0, 'truck': 1, 'bus': 2, 'van': 3, 'vehicles': 4,
                                       'person_on_vehicle': 5, 'static_person': 6, 'distractor':7, 'reflection': 8} # nothing
