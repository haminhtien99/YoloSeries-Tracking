...
def update_track(res: Results,
                 tracker: Sort|BOTSORT|BYTETracker,
                 ims) -> np.ndarray:
    """
    Tracking based predicted per image
    """
    ...
    if isinstance(tracker, Sort):
        ...
        det_to_tracker = np.concatenate((xyxy, score, cls, idx), axis=1)
        if len(det_to_tracker) == 0:
            return np.zeros((0, 8))
        tracks = tracker.update(det_to_tracker)
    elif isinstance(tracker, BYTETracker):
        tracks = tracker.update(det, ims)
    return tracks   # tracks return np.array([[bbox, track_id, score, cls, idx],...])
...
def on_predict_start(predictor: object, persist: bool = False) -> None:
    ...
    if tracker_type not in ['sort', 'botsort', 'bytetrack']:
        raise AssertionError(f"Only 'sort', 'botsort', 'bytetrack' are supported for now, but got '{tracker_type}'")
    trackers = []
...
def on_predict_postprocess_end(predictor: object, persist: bool = False) -> None:
    ...
    for i in range(len(im0s)):
        ...
        det = predictor.results[i]
        tracks = update_track(det, tracker, im0s[i])
        if len(tracks) == 0:
            continue
